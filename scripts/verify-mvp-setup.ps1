...
Write-SubHeader "Step 7/12: Azure AI Foundry"
...
if ($aiAccount) {
    Write-Status -Status success -Message "Azure AI Foundry '$($aiAccount.name)' found"
    ...
    try {
        ...
        if ($deployments -and $deployments.Count -gt 0) {
            ...
            Write-Status -Status warning -Message "No model deployments found. Run setup: docs/AZURE_AI_FOUNDRY_SETUP.md"
        }
    } catch {
        Write-Status -Status warning -Message "Could not list model deployments"
    }
} else {
    Write-Status -Status error -Message "No Azure AI Foundry resource found in resource group"
    Write-Host "  Action: Deploy Azure AI Foundry resource or check resource group"
}
...
if ($script:Failed -eq 0) {
    ...
    Write-Host "  1. Deploy models in Azure AI Foundry (if deployments missing)"
    Write-Host "     See: docs/AZURE_AI_FOUNDRY_SETUP.md"
...
Write-Host ""
Write-Host "For detailed troubleshooting, see: docs/AZURE_AI_FOUNDRY_SETUP.md"
