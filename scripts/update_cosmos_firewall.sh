...
COSMOS_ACCOUNT="${COSMOS_ACCOUNT:-classymail-cosmos}"
API_APP="${API_APP:-classymail-api}"
WORKER_APP="${WORKER_APP:-classymail-worker}"
...
OPTIONS:
    ...
    -c, --cosmos-account <name>   Cosmos DB account name (default: classymail-cosmos)
    -a, --api-app <name>          API Container App name (default: classymail-api)
    -w, --worker-app <name>       Worker Container App name (default: classymail-worker)
...
EXAMPLES:
    ...
    $0 -g classymail-rg --include-local-ip

    # Use environment variables
    export RESOURCE_GROUP=classymail-rg
