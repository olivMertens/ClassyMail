...
API Container App name (default: classymail-api).

.PARAMETER WorkerAppName
Worker Container App name (default: classymail-worker).

.PARAMETER IncludeLocalIP
Include your current public IP in the firewall (for local scripts).

.EXAMPLE
.\scripts\update_cosmos_firewall.ps1 -ResourceGroup "classymail-rg"

.EXAMPLE
.\scripts\update_cosmos_firewall.ps1 -ResourceGroup "classymail-rg" -IncludeLocalIP
#>
...
param(
    ...
    [string]$CosmosAccountName = "classymail-cosmos",
    [string]$ApiAppName = "classymail-api",
    [string]$WorkerAppName = "classymail-worker",
