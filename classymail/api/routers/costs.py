...
async def costs_summary(
    ...
    return {
        ...
        "token_hypothesis": {
            "description": "Classification only — single LLM call per email. Does NOT include entity extraction, PII detection, or email preprocessing.",
            ...
            "cost_multipliers": {
                ...
                "all_features": "~×3–4 total",
            },
        },
        "model_cost_comparison": model_comparison,
        "avg_usd_per_email": {
            "ai_total": float(avg_ai_usd_per_email),
            "mistral_ocr": float((mistral_usd or 0.0) / emails_with_usage) if emails_with_usage else 0.0,
            "llm": float((phi4_usd or 0.0) / emails_with_usage) if emails_with_usage else 0.0,
        },
