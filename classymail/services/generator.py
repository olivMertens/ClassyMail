...
EMAIL_TEMPLATES = {
    "Billing inquiry": [
        ("Demande d'certificate habitation", "Marie Dubois",
         "Je souhaiterais recevoir une certificate habitation pour mon logement situé au 15 rue des Fleurs, 75001 Paris. Cette certificate est nécessaire pour la signature de mon contrat de location prévu le 15 février prochain."),
        ("Attestation urgente", "Jean Martin",
         "Mon agence me réclame une certificate habitation pour mon nouveau logement. Adresse : 42 Avenue de la République, 69003 Lyon. Merci de me la faire parvenir rapidement."),
    ],
    "Cancellation": [
        ("Demande de cancellation de contrat", "Sophie Laurent",
         "Je souhaite résilier mon contrat d'home service n° HAB-12345 à compter du 31 mars 2026. Je déménage à l'étranger pour raisons professionnelles."),
        ("Cancellation suite déménagement", "Pierre Durand",
         "Suite à mon déménagement, je vous informe de ma volonté de résilier mon contrat d'service auto n° AUTO-98765 au 15 février 2026."),
    ],
    "Service dégât des eaux": [
        ("Déclaration service issue dégât des eaux", "Claire Rousseau",
         "Je déclare un service issue dégât des eaux survenu le 25 janvier 2026 dans mon appartement. L'eau provient de l'étage supérieur et a endommagé mon salon et ma chambre. Contrat : HAB-54321."),
    ...
    "Quote request": [
        ("Devis service auto", "Thomas Petit",
        ...
        ("Devis habitation", "Emma Moreau",
         "Pourriez-vous m'établir un devis pour une home service ? Appartement de 65m², 2 pièces, Paris 18ème. Locataire. Besoin couverture complète."),
    ],
    "Modification contrat": [
        ...
        ("Ajout conducteur secondaire", "Julie Bernard",
         "Je souhaite ajouter mon conjoint comme conducteur secondaire sur mon contrat auto n° AUTO-22222. Permis obtenu en 2020, aucun service issue."),
    ],
}
...
def generate_email_pdf(subject: str = None, sender: str = None, body: str = None, category: str = None, use_aoai: bool = False) -> tuple[bytes, str, str]:
    ...
    category : str, optional
        Business category (for template selection). If None, random category chosen.
    ...
    pdf, cat, subj = generate_email_pdf(category="Service dégât des eaux", use_aoai=True)
    """
    try:
...
def _aoai_enhance_body(body: str, category: str) -> Optional[str]:
    ...
    prompt = (
        f"Enhance this email body with more realistic details, "
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "You enhance email bodies with realistic details. "
...
async def generate_synthetic_from_seeds(seed_examples: list[dict], count: int = 5) -> list[dict]:
    ...
    while generated_count < count:
        ...
        system_prompt = (
            "You are a synthetic data generator for an email processing system. "
