...
def test_parse_suggestion_french_rewrite_definition():
    ...
    text = "RÉÉCRIRE le champ 'Définition' (remplacer tout le contenu actuel par): DEFINITION Billing inquiry: document officiel. Mots-clés: bail, quittance"
    result = _parse_suggestion(text)
    assert result["action"] == "rewrite"
    assert result["field"] == "definition"
    assert result["content"].startswith("Billing inquiry")
