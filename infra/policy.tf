...
variable "tag_policy_enabled" {
  type        = bool
  description = "Enable the tag auto-fill policy assignment (corporate tags: cp-code-sa, cp-deploiement, cp-environnement, cp-proprietaire, cp-responsable, cp-supervision)."
  default     = true
}
...
resource "azurerm_policy_definition" "add_mandatory_tags" {
  count        = var.custom_tags_enabled ? 1 : 0
  name         = "add-mandatory-tags"
  policy_type  = "Custom"
  mode         = "All"
  display_name = "Add ClassyMail Mandatory Tags"
  description  = "This policy automatically adds mandatory corporate tags (cp-code-sa, cp-deploiement, cp-environnement, cp-proprietaire, cp-responsable, cp-supervision) to all resources except subscriptions, resource groups, deployments, and management groups."
...
resource "azurerm_resource_group_policy_assignment" "add_mandatory_tags_rg" {
  count                = var.custom_tags_enabled && var.tag_policy_enabled && var.tag_policy_scope == "resource_group" ? 1 : 0
  name                 = "add-mandatory-tags"
  resource_group_id    = azurerm_resource_group.rg.id
  policy_definition_id = azurerm_policy_definition.add_mandatory_tags[0].id
...
resource "azurerm_subscription_policy_assignment" "add_mandatory_tags_sub" {
  count                = var.custom_tags_enabled && var.tag_policy_enabled && var.tag_policy_scope == "subscription" ? 1 : 0
  name                 = "add-mandatory-tags"
...
resource "azurerm_role_assignment" "add_mandatory_tags_role_rg" {
  count = var.custom_tags_enabled && var.tag_policy_enabled && var.tag_policy_scope == "resource_group" ? 1 : 0
  ...
  principal_id       = azurerm_resource_group_policy_assignment.add_mandatory_tags_rg[0].identity[0].principal_id
}

resource "azurerm_role_assignment" "add_mandatory_tags_role_sub" {
  count = var.custom_tags_enabled && var.tag_policy_enabled && var.tag_policy_scope == "subscription" ? 1 : 0

  scope              = local.tag_policy_sub_scope_id
  role_definition_id = "/providers/Microsoft.Authorization/roleDefinitions/b24988ac-6180-42a0-ab88-20f7382dd24c"
  principal_id       = azurerm_subscription_policy_assignment.add_mandatory_tags_sub[0].identity[0].principal_id
}

# Optional: remediate existing resources that are already missing the tags.
resource "azurerm_resource_group_policy_remediation" "add_mandatory_tags_rg" {
  count = var.custom_tags_enabled && var.tag_policy_enabled && var.tag_policy_scope == "resource_group" ? 1 : 0

  name                 = "remediate-add-classymail-tags"
  resource_group_id    = azurerm_resource_group.rg.id
  policy_assignment_id = azurerm_resource_group_policy_assignment.add_mandatory_tags_rg[0].id

  depends_on = [azurerm_role_assignment.add_mandatory_tags_role_rg]
}

resource "azurerm_subscription_policy_remediation" "add_mandatory_tags_sub" {
  count = var.custom_tags_enabled && var.tag_policy_enabled && var.tag_policy_scope == "subscription" ? 1 : 0

  name                 = "remediate-add-classymail-tags"
  subscription_id      = data.azurerm_subscription.current.subscription_id
  policy_assignment_id = azurerm_subscription_policy_assignment.add_mandatory_tags_sub[0].id

  depends_on = [azurerm_role_assignment.add_mandatory_tags_role_sub]
}
