...
if (correctionReason.value.length < 5) {
  errorMessage.value = "Please provide a reason or comment for marking this email as Invalid (at least 5 characters).";
  return;
}
...
**Example AI Feedback**:
```
"The model classified this service issue as MUST KEEP when it is a
cancellation request (MUST GO). The presence of 'terminate my contract'
should have triggered the CANCELLATION intent."
```
...
⚠️ **Note**: Microsoft AI Foundry supports an optional `weight` parameter on assistant messages for **skipping specific messages** during training.
...
**Reference**: [Microsoft AI Foundry Fine-Tuning Docs](https://learn.microsoft.com/en-us/azure/ai-foundry/openai/how-to/fine-tuning?view=foundry-classic&tabs=oai-sdk%2Cazure-openai&pivots=programming-language-python#multiple-turn-chat-file-format)
...
```json
{
  "messages": [
    {"role": "system", "content": "..."},
    {"role": "user", "content": "Subject: Cancellation\n\nI would like to cancel..."},
  ...
  "metadata": {
    ...
    "correction": {
      "was_corrected": true,
      "correction_reason": "The model missed the keyword 'cancel' which should trigger CANCELLATION",
      "llm_feedback": "The model misinterpreted an explicit cancellation request as a simple information request.",
...
# 1. "The model misses cancellation requests" → 15 occurrences
# 2. "The model confuses MUST GO and MUST KEEP" → 12 occurrences
# 3. "The model ignores attachments" → 8 occurrences

# Update system prompt:
SYSTEM_PROMPT += """
SPECIAL ATTENTION:
- Words like "cancel", "terminate", "end" indicate CANCELLATION + MUST GO
- When in doubt between MUST GO and MUST KEEP, prioritize the context of attachments
- Cancellation requests are ALWAYS MUST GO, even if the tone is polite
"""
```
...
- **System Prompt**: Generic 1-line prompt: `"You classify business emails into intents and output strict JSON only."`
...
**1. System Prompt** (matches production exactly — prompt is in French as the system is configured for French-language email classification):
```python
...
- Choose the intents whose DEFINITION best matches the content.
- EXCLUSIONS specify what each category must NOT include.
- Un email peut contenir UNE SEULE intention OU PLUSIEURS intentions.
- If no intent matches, return an empty list. Do NOT guess.
...
```json
{
  ...
  "classification_reason": "No intent matches (empty intents only)",
  "subject": "Cancellation request for home insurance",
...
```json
{
  "subject": "Cancellation request for home insurance by [Email]",
  "sender": "[Email]"
}
```
...
system_prompt = "You classify business emails into intents and output strict JSON only."
...
async def export_finetune_jsonl_iter(
    *, clients, anonymize, include_unreviewed, max_examples,
    ...
    # Emit UTF-8 BOM (required by Microsoft AI Foundry)
    yield "\ufeff"
...
- [PII_ANONYMIZATION_AND_USER_CORRECTIONS.md](PII_ANONYMIZATION_AND_USER_CORRECTIONS.md): Fine-tuning guide and PII protection
...
**Reference**: [Microsoft AI Foundry Multiple-Turn Chat Format](https://learn.microsoft.com/en-us/azure/ai-foundry/openai/how-to/fine-tuning?view=foundry-classic&tabs=oai-sdk%2Cazure-openai&pivots=programming-language-python#multiple-turn-chat-file-format)
...
- [AI_FOUNDRY_SETUP.md](AI_FOUNDRY_SETUP.md) - Microsoft AI Foundry setup for fine-tuning
...
- **2026-02-06 (v1.1)**: Verified against [official Microsoft AI Foundry documentation](https://learn.microsoft.com/en-us/azure/ai-foundry/openai/how-to/fine-tuning). Added clarification on two "weight" concepts: Microsoft's binary `weight` parameter (0/1) vs our metadata-based implicit weighting. Added format validation section and optional enhancement guidance. Confirmed full compatibility with Azure OpenAI fine-tuning requirements.
