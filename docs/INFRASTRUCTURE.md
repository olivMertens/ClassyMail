...
> ??? **Comprehensive Guide**: Terraform provisioning, resource configuration, Event Grid setup, RBAC, and network strategy.
>
...
??? Resource tagging is **optional** and configurable. By default, corporate tags are applied to all resources.

> **tagged deployments**: See [CUSTOMIZATION.md](CUSTOMIZATION.md#classymail-mandatory-tags) for the full tag table and Azure Policy enforcement.
>
> **Non-tagged deployments**: Set `custom_tags_enabled = false` in `terraform.tfvars` to disable corporate tags.
...
|-------|----------------|---------------------|---------|----------|
| **Mistral Document AI 2505** | `mistral-document-ai-2512` | `MISTRAL_DEPLOYMENT` | OCR + Vision extraction | ? MANDATORY |
...
| **GPT-5.2-chat** | `gpt-5.2-chat` | `CHAT_DEPLOYMENT` | Chatbot conversational AI | ?? RECOMMENDED |
| **GPT-5-nano** | `gpt-5-nano` | *(hardcoded in category_assessment.py)* | Category assessment AI | ?? RECOMMENDED |
...
**Cost:** ~ï¿½1.00 per 1,000 text records (1 email = 1 record) ? ~ï¿½0.001/email

**When to Use:**
- ? Compliance requirements (GDPR, HIPAA, industry regulatory)
- ? Need for 43+ predefined PII categories
- ? Budget allows (~ï¿½0.001/email fixed cost)
...
- **SKU:** `S0` (Standard) or `F0` (Free ï¿½ 500 pages/month)
...
**Usage:** Automatic ï¿½ pipeline falls back to Document Intelligence when Mistral OCR fails. No UI configuration needed.
...
**?? Security Note:** Do NOT set `AZURE_AI_KEY` in production. Use Managed Identity authentication (`DefaultAzureCredential`) with the `Cognitive Services User` role.
...
> ?? **Cosmos DB uses a Custom Role** (not the built-in `Cosmos DB Built-in Data Contributor`). The custom role grants `readMetadata` + specific CRUD actions at **Account scope**. See [RBAC_AUDIT.md](RBAC_AUDIT.md) ï¿½9 for why.
...
> ?? The scope **must** be the account ID (not `/dbs/emailsdb`), because the Python SDK calls `readMetadata` at account level on initialization.
...
**Permanent fix:** Run `terraform apply` ï¿½ the custom role definition and assignment are managed in `main.tf`.
