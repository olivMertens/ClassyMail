# Documentation

## Navigation

- [ARCHITECTURE](ARCHITECTURE.md)
- [PIPELINE](PIPELINE.md)
- [TERRAFORM](TERRAFORM.md)
- [MODELS](MODELS.md)
- [FINE_TUNING_DATA](FINE_TUNING_DATA.md)
- [CICD_GITHUB](CICD_GITHUB.md)
- [CICD_GITLAB](CICD_GITLAB.md)
- [LOCAL_RUN](LOCAL_RUN.md)
- [SCENARIO_E2E](SCENARIO_E2E.md)
- [DEV_LOCAL_BUILD](DEV_LOCAL_BUILD.md)
- [USER_INTERFACE](USER_INTERFACE.md)
- [TESTING_EMAIL_GENERATION](TESTING_EMAIL_GENERATION.md) - Generate realistic email PDFs for testing

## Liens rapides

- Exécuter en local : voir [LOCAL_RUN](LOCAL_RUN.md)
- Scénario de test bout en bout : voir [SCENARIO_E2E](SCENARIO_E2E.md)
- Générer des emails de test : voir [TESTING_EMAIL_GENERATION](TESTING_EMAIL_GENERATION.md)
- Build & push sans CI : voir [DEV_LOCAL_BUILD](DEV_LOCAL_BUILD.md)
- Script de déploiement infra : voir [infra/deploy.ps1](../infra/deploy.ps1)
