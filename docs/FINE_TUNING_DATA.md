...
> **Catégories ClassyMail** : Voir [CUSTOMIZATION.md](CUSTOMIZATION.md#insurance-category-taxonomy) pour la taxonomie complète (Attestation, Résiliation, Sinistre, etc.).
...
> **System prompt**: The system prompt is built dynamically from the production category taxonomy stored in settings (see `get_categories_prompt_text_async()` in `settings_store.py`). It includes the full list of ClassyMail categories with descriptions. — See [CUSTOMIZATION.md](CUSTOMIZATION.md#fine-tuning-with-classymail-categories) for details.
>
