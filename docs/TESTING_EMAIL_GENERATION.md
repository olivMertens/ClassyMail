...
```bash
uv run python scripts/generate_realistic_emails.py --count 5 --categories "Billing inquiry" "Cancellation"
```
...
> **ClassyMail categories**: See [CUSTOMIZATION.md](CUSTOMIZATION.md#business-category-taxonomy) for the full category taxonomy (Billing inquiry, Cancellation, Service issue, etc.).
>
...
2. **Upload via API:**
   ...
   curl -X POST http://localhost:8000/api/upload \
     ...
     -F "file=@dataset/pdf/sample_document_20260127_123456.pdf"
   ```
...
### Generate only certificate and cancellation emails

```bash
uv run python scripts/generate_realistic_emails.py --count 30 \
  --categories "Billing inquiry" "Cancellation"
```
...
- [INTEGRATION.md](./INTEGRATION.md) - Full end-to-end integration scenarios
