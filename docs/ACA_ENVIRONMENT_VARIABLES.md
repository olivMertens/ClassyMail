...
- **`MISTRAL_DEPLOYMENT`** - The deployment name for Mistral OCR model
  ...
  - ⚠️ **CRITICAL:** Must be EXACTLY `mistral-document-ai-2512` — typos (e.g., `mistral-ocr-2505`) cause HTTP 500 errors during OCR processing. Verify in Azure AI Foundry → Deployments.
...
- **`ORGANIZATION_NAME`** - Organization name displayed in UI
  - Example: `My Company`
  - Default: `ClassyMail`
  - Note: tagged deployments set this to `ClassyMail` (see [CUSTOMIZATION.md](CUSTOMIZATION.md))
