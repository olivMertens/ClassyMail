  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 gap-5 sm:grid-cols-3">
       <div class="bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg px-4 py-5 sm:p-6">
          <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">Total Emails</dt>
          <dd class="mt-1 text-3xl font-semibold text-gray-900 dark:text-white">{{ stats.total }}</dd>
       </div>
       <div class="bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg px-4 py-5 sm:p-6">
          <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">To Review</dt>
          <dd class="mt-1 text-3xl font-semibold text-amber-600 dark:text-amber-400">{{ stats.review_required }}</dd>
       </div>
       <div class="bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg px-4 py-5 sm:p-6">
          <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">Processed</dt>
          <dd class="mt-1 text-3xl font-semibold text-green-600 dark:text-green-400">{{ stats.processed }}</dd>
       </div>
    </div>
    ...
    <div v-if="loading && !emails.length" class="text-center py-12">
        ...
        <p class="mt-4 text-gray-500 dark:text-gray-400">Loading emails...</p>
    </div>
       ...
       <div v-for="email in emails" :key="email.id" 
          ...
          <div class="p-5 flex-1">
             <div class="flex justify-between items-start mb-2">
                 <span class="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset" :class="getScoreColor(email)">
                    Score {{ getScore(email) }}
                 </span>
                 <span v-if="email.status === 'ERROR'" class="text-red-500" title="Error">
                    <ExclamationCircleIcon class="h-5 w-5" />
                 </span>
                 <span v-else-if="email.status === 'PROCESSED'" class="text-green-500" title="Processed">
                    <CheckCircleIcon class="h-5 w-5" />
                 </span>
                 <span v-else-if="email.status === 'REVIEW_REQUIRED'" class="text-amber-500" title="Review Required">
                    <ClockIcon class="h-5 w-5" />
                 </span>
             </div>
             
             <h3 class="text-lg font-medium text-gray-900 dark:text-white truncate mb-1" :title="email.subject">
             ...
             <div v-if="email.error" class="mt-2 text-xs text-red-600 dark:text-red-400 line-clamp-2">
                 {{ email.error }}
             </div>
          </div>
          ...
          <div class="bg-gray-50 dark:bg-gray-700/50 px-5 py-3 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center rounded-b-lg">
              <button @click="emit('open-email', email)" class="text-sm font-medium text-primary-600 hover:text-primary-500 dark:text-primary-400">
                  Open Details
              </button>
    ...
    <div class="flex items-center justify-between border-t border-gray-200 dark:border-gray-700 pt-4">
        <div class="flex items-center">
            <select v-model="pageSize" class="block w-full rounded-md border-0 py-1.5 pl-3 pr-10 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-primary-600 sm:text-sm sm:leading-6 dark:bg-gray-800 dark:ring-gray-700 dark:text-white">
                <option v-for="opt in pageSizeOptions" :key="opt" :value="opt">{{ opt }} / page</option>
            </select>
        </div>
        <div class="flex items-center gap-2">
            <button @click="page--" :disabled="page <= 1" class="p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-50 text-gray-500 dark:text-gray-400">
                <ChevronLeftIcon class="h-5 w-5" />
            </button>
            <span class="text-sm text-gray-700 dark:text-gray-300">
                Page <span class="font-medium">{{ page }}</span> of <span class="font-medium">{{ totalPages }}</span>
            </span>
            <button @click="page++" :disabled="page >= totalPages" class="p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-50 text-gray-500 dark:text-gray-400">
