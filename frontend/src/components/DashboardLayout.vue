...
const uiConfig = ref({ show_info_modal: true, show_developer_tab: true, organization_name: 'ClassyMail', environment: 'development', commit_sha: null, build_timestamp: null })
...
<template>
  <div class="h-full min-h-screen bg-surface">
    <!-- Mobile sidebar backdrop -->
    <div v-if="sidebarOpen" class="fixed inset-0 z-40 bg-gray-600 bg-opacity-75 md:hidden"
      @click="sidebarOpen = false" />

    <!-- Mobile sidebar -->
    <div v-if="sidebarOpen" class="fixed inset-y-0 left-0 z-50 w-72 bg-sidebar shadow-xl md:hidden flex flex-col">
    ...
    <!-- Desktop sidebar -->
    <div class="hidden md:fixed md:inset-y-0 md:flex md:flex-col shadow-lg z-30"
      :class="sidebarCollapsed ? 'md:w-20' : 'md:w-52'">
      <div class="flex flex-col flex-1 min-h-0 bg-sidebar border-r border-surface">
        <div class="flex items-center h-16 flex-shrink-0 px-4 bg-sidebar border-b border-surface">
          <span class="text-2xl mr-2">📧</span>
          <span v-if="!sidebarCollapsed" class="text-xl font-bold text-gray-900 dark:text-white">{{ organizationName
          }}</span>
        </div>
        <div class="flex-1 flex flex-col overflow-y-auto">
          <nav class="flex-1 px-2 py-4 space-y-1" :class="sidebarCollapsed ? 'items-center' : ''">
            <a v-for="item in navigation" :key="item.name" href="#"
    ...
    <!-- Content Area -->
    <div class="flex flex-col flex-1 h-screen overflow-hidden" :class="sidebarCollapsed ? 'md:pl-20' : 'md:pl-52'">
      <!-- Top bar -->
      <div class="sticky top-0 z-10 flex-shrink-0 flex h-16 bg-topbar shadow dark:shadow-gray-900/20 md:hidden">
        <button
        ...
        <div class="flex-1 px-4 flex justify-between items-center">
          ...
          <div class="flex items-center gap-2">
            <button v-if="uiConfig.show_info_modal"
      ...
      <header class="hidden md:flex items-center justify-between h-16 px-4 bg-topbar border-b border-surface gap-2">
        <button
          class="p-2 rounded-full text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700 transition-colors"
          :title="t('layout.toggle_sidebar')" @click="sidebarCollapsed = !sidebarCollapsed">
          <Bars3Icon class="h-6 w-6" />
        </button>
        <div class="flex items-center gap-2">
          <button v-if="uiConfig.show_info_modal"
            class="p-2 rounded-full text-blue-400 hover:text-blue-300 dark:text-blue-400 dark:hover:text-blue-300 transition-colors"
            :title="t('layout.info')" @click="showInfoModal = true">
            <InformationCircleIcon class="h-6 w-6" />
          </button>
          <button
            class="p-2 rounded-full text-purple-400 hover:text-purple-300 dark:text-purple-400 dark:hover:text-purple-300 transition-colors"
            @click="toggleDarkMode">
            <SunIcon v-if="isDark" class="h-6 w-6" />
            <MoonIcon v-else class="h-6 w-6" />
          </button>
    ...
    <InfoModal :show="showInfoModal" :organization-name="organizationName" :commit-sha="uiConfig.commit_sha"
      :build-timestamp="uiConfig.build_timestamp" @close="showInfoModal = false"
